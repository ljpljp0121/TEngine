    ...
    public partial class BaseGraphProcessor
    {
        ...
        private void BeginInitConnections()
        {
            ...
            for (int i = 0; i < Model.connections.Count; i++)
            {
                ...
                {
                    ReportDiagnostic(
                        $"[MissingConnection] fromNode={connection.fromNode} not found, removed ({connection.fromPort}->{connection.toNode}:{connection.toPort}).");
                    Model.connections.RemoveAt(i--);
                    continue;
                }
                ...
                {
                    ReportDiagnostic(
                        $"[MissingConnection] fromPort '{connection.fromPort}' missing on node {connection.fromNode}, connection removed.");
                    Model.connections.RemoveAt(i--);
                    continue;
                }
                ...
                {
                    ReportDiagnostic(
                        $"[MissingConnection] toNode={connection.toNode} not found, removed ({connection.fromNode}:{connection.fromPort}->{connection.toPort}).");
                    Model.connections.RemoveAt(i--);
                    continue;
                }
                ...
                {
                    ReportDiagnostic(
                        $"[MissingConnection] toPort '{connection.toPort}' missing on node {connection.toNode}, connection removed.");
                    Model.connections.RemoveAt(i--);
                    continue;
                }
                ...
                {
                    ReportDiagnostic(
                        $"[InvalidConnection] {error}, removed ({connection.fromNode}:{connection.fromPort}->{connection.toNode}:{connection.toPort}).");
                    Model.connections.RemoveAt(i--);
                    continue;
                }
                ...
                {
                    ReportDiagnostic(
                        $"[DuplicateConnection] Duplicate edge {connection.fromNode}:{connection.fromPort}->{connection.toNode}:{connection.toPort} removed.");
                    Model.connections.RemoveAt(i--);
                    continue;
                }
        ...
        public void Disconnect(BaseConnectionProcessor connection)
        {
            ...
            graphEvents.Publish(new RemoveConnectionEventArgs(connection));
        }
        ...
        private void InternalConnect(PortProcessor fromPort, PortProcessor toPort, BaseConnectionProcessor connection)
        {
            ...
            graphEvents.Publish(new AddConnectionEventArgs(connection));
        }
