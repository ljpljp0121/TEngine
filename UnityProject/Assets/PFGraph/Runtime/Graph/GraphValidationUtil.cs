    ...
    public static class GraphValidationUtil
    {
        ...
        private static HashSet<long> ValidateNodes(BaseGraph graph, GraphValidationResult result, bool repair)
        {
            ...
            for (var i = 0; i < graph.nodes.Count; i++)
            {
                ...
                {
                    result.Add($"[DuplicateNode] Node id={node.id} duplicated." +
                               (repair ? " Later entry removed." : string.Empty));
                    if (repair)
                        graph.nodes.RemoveAt(i--);
                }
            }
        ...
        private static void ValidateConnections(BaseGraph graph, HashSet<long> nodeIds, GraphValidationResult result,
            bool repair)
        {
            ...
            for (var i = 0; i < graph.connections.Count; i++)
            {
                ...
                if (connection == null)
                {
                    result.Add($"[MissingConnection] Null connection at index {i}." +
                               (repair ? " Removed." : string.Empty));
                    if (repair)
                ...
                {
                    result.Add(
                        $"[MissingConnection] Missing endpoint node for {connection.fromNode}:{connection.fromPort}->{connection.toNode}:{connection.toPort}." +
                        (repair ? " Removed." : string.Empty));
                    if (repair)
                        graph.connections.RemoveAt(i--);
                    continue;
                ...
                var key = connection.fromNode + ":" + connection.fromPort + "->" + connection.toNode + ":" +
                          connection.toPort;
                if (!connectionKeys.Add(key))
                {
                    result.Add($"[DuplicateConnection] Duplicate edge {key}." +
                               (repair ? " Later entry removed." : string.Empty));
                    if (repair)
                        graph.connections.RemoveAt(i--);
                }
        ...
        private static void ValidateGroups(BaseGraph graph, HashSet<long> nodeIds, GraphValidationResult result,
            bool repair)
        {
            ...
            for (var i = 0; i < graph.groups.Count; i++)
            {
                ...
                {
                    result.Add($"[DuplicateGroup] Group id={group.id} duplicated." +
                               (repair ? " Later entry removed." : string.Empty));
                    if (repair)
                        graph.groups.RemoveAt(i--);
                    continue;
                ...
                if (group.nodes == null)
                {
                    result.Add($"[InvalidGroup] Group id={group.id} nodes list was null." +
                               (repair ? " Recreated." : string.Empty));
                    if (repair)
                ...
                for (var j = 0; j < group.nodes.Count; j++)
                {
                    ...
                    {
                        result.Add($"[InvalidGroup] Group id={group.id} contains missing node {nodeId}." +
                                   (repair ? " Removed." : string.Empty));
                        if (repair)
                    ...
                    {
                        result.Add($"[InvalidGroup] Group id={group.id} contains duplicated node {nodeId}." +
                                   (repair ? " Later entry removed." : string.Empty));
                        if (repair)
                            group.nodes.RemoveAt(j--);
                    }
        ...
        private static void ValidateStickyNotes(BaseGraph graph, GraphValidationResult result, bool repair)
        {
            ...
            for (var i = 0; i < graph.notes.Count; i++)
            {
                ...
                if (!ids.Add(note.id))
                {
                    result.Add($"[DuplicateNote] Note id={note.id} duplicated." +
                               (repair ? " Later entry removed." : string.Empty));
                    if (repair)
                        graph.notes.RemoveAt(i--);
                }
        ...
        private static void ValidatePlacemats(BaseGraph graph, GraphValidationResult result, bool repair)
        {
            ...
            for (var i = 0; i < graph.placemats.Count; i++)
            {
                ...
                if (placemat == null)
                {
                    result.Add($"[MissingPlacemat] Null placemat at index {i}." +
                               (repair ? " Removed." : string.Empty));
                    if (repair)
                ...
                {
                    result.Add($"[DuplicatePlacemat] Placemat id={placemat.id} duplicated." +
                               (repair ? " Later entry removed." : string.Empty));
                    if (repair)
                        graph.placemats.RemoveAt(i--);
                }
