...
namespace PFGraph
{
    ...
    public abstract class BaseGraphWindow : BaseEditorWindow
    {
        ...
        private readonly List<KeyValuePair<string, object>> blackboardEntries =
            new List<KeyValuePair<string, object>>(64);
        ...
        protected void Load(BaseGraphProcessor graph, IGraphOwner graphOwner, IGraphAsset graphAsset)
        {
            ...
            this.context = new GraphViewContext()
                { graphWindow = this, commandDispatcher = new CommandDispatcher(historyLimit) };
        ...
        {
            Load(
                ViewModelFactory.ProduceViewModel(GraphProcessorEditorUtil.Clone(graphAsset.LoadGraph())) as
                    BaseGraphProcessor, null, graphAsset);
        }
        ...
        {
            GraphAsset?.SaveGraph(GraphProcessorEditorUtil.Clone(GraphProcessor.Model));
            if (UnityGraphAsset)
        ...
        protected virtual void BuildToolBar()
        {
            ...
            togMiniMap.clicked += () =>
            {
                GraphProcessorEditorSettings.MiniMapActive.Value =
                    !GraphProcessorEditorSettings.MiniMapActive.Value;
            };
            ...
            togSnap.clicked += () =>
            {
                GraphProcessorEditorSettings.GridSnapActive.Value =
                    !GraphProcessorEditorSettings.GridSnapActive.Value;
            };
            ...
            togInspector.clicked += () =>
            {
                GraphProcessorEditorSettings.InspectorActive.Value =
                    !GraphProcessorEditorSettings.InspectorActive.Value;
            };
            ...
            togItemLibrary.clicked += () =>
            {
                GraphProcessorEditorSettings.ItemLibraryActive.Value =
                    !GraphProcessorEditorSettings.ItemLibraryActive.Value;
            };
        ...
        private void RefreshBreadcrumbs()
        {
            ...
            var assetName = unityGraphAsset != null
                ? unityGraphAsset.name
                : (GraphProcessor?.ModelType?.Name ?? "Graph");
            if (subgraphBreadcrumbStack.Count == 0)
            {
        ...
        private void BuildInspectorPanel()
        {
            ...
            {
                if (element is Label label && itemLibraryFiltered != null && index >= 0 &&
                    index < itemLibraryFiltered.Count)
                    label.text = itemLibraryFiltered[index].Path;
            };
#if UNITY_2022_1_OR_NEWER
            itemLibraryList.itemsChosen += _ =>
#else
            itemLibraryList.onItemsChosen += _ =>
#endif
            {
                if (itemLibraryList.selectedIndex < 0 || itemLibraryFiltered == null ||
                    itemLibraryList.selectedIndex >= itemLibraryFiltered.Count)
                    return;
        ...
        private void RefreshInspectorPanel()
        {
            ...
            {
                var diagnosticsFoldout = new Foldout()
                    { text = $"Missing Item / Error ({diagnostics.Count})", value = true };
            ...
            if (subgraphAssets.Count > 0)
            {
                ...
                for (var i = 0; i < subgraphAssets.Count; i++)
                {
                    ...
                    btnOpenSubgraph.text =
                        $"Open {(subgraphAsset as UnityObject)?.name ?? subgraphAsset.GraphType.Name}";
            ...
            foreach (var item in GraphView.selection)
            {
                ...
                if (item is BaseNodeView node)
                    ...
                    inspectorBody.Add(new Label(
                        $"{placemat.ViewModel.Title} [{placemat.ViewModel.Size.x}x{placemat.ViewModel.Size.y}]"));
                else if (item is BaseConnectionView connection)
                    inspectorBody.Add(new Label(
                        $"{connection.ViewModel.FromNodeID}:{connection.ViewModel.FromPortName} -> {connection.ViewModel.ToNodeID}:{connection.ViewModel.ToPortName}"));
            }
        ...
        private void OnSetBlackboardClicked()
        {
            ...
            context?.Do(() => { GraphProcessor.Blackboard.Set(key, newValue); }, () =>
            {
                if (oldExists)
        ...
        private void OnRemoveBlackboardClicked()
        {
            ...
            context?.Do(() => { GraphProcessor.Blackboard.Remove(key); },
                () => { GraphProcessor.Blackboard.Set(key, oldValue); });
