...
namespace PFGraph
{
    public sealed class StickyNoteView : UnityEditor.Experimental.GraphView.StickyNote,
        IGraphElementView<StickyNoteProcessor>
    {
        ...
        public void Init()
        {
            ViewModel.RegisterValueChanged<InternalVector2Int>(nameof(PFGraph.StickyNote.position), OnPositionChanged);
            ViewModel.RegisterValueChanged<InternalVector2Int>(nameof(PFGraph.StickyNote.size), OnSizeChanged);
            ViewModel.RegisterValueChanged<string>(nameof(PFGraph.StickyNote.title), OnTitleChanged);
            ViewModel.RegisterValueChanged<string>(nameof(PFGraph.StickyNote.content), OnContentsChanged);
        ...
        public void UnInit()
        {
            ViewModel.UnregisterValueChanged<InternalVector2Int>(nameof(PFGraph.StickyNote.position),
                OnPositionChanged);
            ViewModel.UnregisterValueChanged<InternalVector2Int>(nameof(PFGraph.StickyNote.size), OnSizeChanged);
            ViewModel.UnregisterValueChanged<string>(nameof(PFGraph.StickyNote.title), OnTitleChanged);
            ViewModel.UnregisterValueChanged<string>(nameof(PFGraph.StickyNote.content), OnContentsChanged);
        ...
        private void OnChanged(StickyNoteChangeEvent evt)
        {
            switch (evt.change)
            {
                ...
                case StickyNoteChange.Contents:
                {
                    ...
                    Owner.Context.Do(() => { ViewModel.Content = newContent; },
                        () => { ViewModel.Content = oldContent; });
                    break;
                }
