...
namespace PFGraph
{
    public partial class BaseGraphView : GraphView
    {
        ...
        public Dictionary<BaseConnectionProcessor, BaseConnectionView> ConnectionViews { get; } =
            new Dictionary<BaseConnectionProcessor, BaseConnectionView>();
        ...
        private IEnumerator GenerateConnectionViews()
        {
            for (var index = 0; index < ViewModel.Connections.Count; index++)
            {
                ...
                if (!NodeViews.TryGetValue(connection.ToNodeID, out var toNodeView))
                    throw new InvalidOperationException($"找不到To节点View: {connection.ToNodeID}");
        ...
        {
            var placematView = this.placematContainer.CreatePlacemat<PlacematView>(
                new Rect(placemat.Position.ToVector2(), placemat.Size.ToVector2()), 0, placemat.Title);
        ...
        private void NodeCreationRequest(NodeCreationContext c)
        {
            ...
            if (multiLayereEntryCount > 0)
                nodeMenu.entries.QuickSort(0, multiLayereEntryCount - 1,
                    (a, b) => String.Compare(a.Path, b.Path, StringComparison.Ordinal));
            if (multiLayereEntryCount < nodeMenu.entries.Count)
                nodeMenu.entries.QuickSort(multiLayereEntryCount, nodeMenu.entries.Count - 1,
                    (a, b) => String.Compare(a.Path, b.Path, StringComparison.Ordinal));
