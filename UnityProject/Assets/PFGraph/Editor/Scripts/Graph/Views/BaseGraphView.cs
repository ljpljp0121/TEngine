...
namespace PFGraph
{
    public abstract partial class BaseGraphView
    {
        private sealed class GraphClipboardData
        {
            ...
            public readonly List<PFGraph.Group> Groups = new List<PFGraph.Group>();
            public readonly List<PFGraph.StickyNote> Notes = new List<PFGraph.StickyNote>();
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            evt.menu.AppendAction("Create Group", delegate
                ...
                (DropdownMenuAction a) => canDeleteSelection && selection.Find(s => s is BaseNodeView) != null
            ...
            {
                var data = new PFGraph.StickyNote();
            ...
            evt.menu.AppendAction("Create Placemat", delegate
            {
                ...
                this.Context.Do(() => { ViewModel.AddPlacemat(placemat); },
                    () => { ViewModel.RemovePlacemat(placemat.ID); });
            });

            evt.menu.AppendSeparator();
            evt.menu.AppendAction("Selection/Frame", _ => FrameSelection(),
                _ => selection.Count > 0 ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);
            evt.menu.AppendAction("Selection/Copy", _ => CopySelectionToClipboard(),
                _ => selection.Count > 0 ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);
            evt.menu.AppendAction("Selection/Cut", _ => CutSelectionToClipboard(),
                _ => selection.Count > 0 ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);
            evt.menu.AppendAction("Selection/Paste", _ => PasteClipboard(),
                _ => HasClipboardData() ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);
            evt.menu.AppendAction("Selection/Duplicate", _ => DuplicateSelection(),
                _ => selection.Count > 0 ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);
            evt.menu.AppendAction("Selection/Align Left", _ => AlignSelection(SelectionAlignMode.Left),
                _ => HasAtLeastTwoSelectedScopes()
                    ? DropdownMenuAction.Status.Normal
                    : DropdownMenuAction.Status.Disabled);
            evt.menu.AppendAction("Selection/Align Right", _ => AlignSelection(SelectionAlignMode.Right),
                _ => HasAtLeastTwoSelectedScopes()
                    ? DropdownMenuAction.Status.Normal
                    : DropdownMenuAction.Status.Disabled);
            evt.menu.AppendAction("Selection/Align Top", _ => AlignSelection(SelectionAlignMode.Top),
                _ => HasAtLeastTwoSelectedScopes()
                    ? DropdownMenuAction.Status.Normal
                    : DropdownMenuAction.Status.Disabled);
            evt.menu.AppendAction("Selection/Align Bottom", _ => AlignSelection(SelectionAlignMode.Bottom),
                _ => HasAtLeastTwoSelectedScopes()
                    ? DropdownMenuAction.Status.Normal
                    : DropdownMenuAction.Status.Disabled);
            evt.menu.AppendAction("Selection/Distribute Horizontal", _ => DistributeSelectionHorizontal(),
                _ => HasAtLeastThreeSelectedScopes()
                    ? DropdownMenuAction.Status.Normal
                    : DropdownMenuAction.Status.Disabled);
            evt.menu.AppendAction("Selection/Distribute Vertical", _ => DistributeSelectionVertical(),
                _ => HasAtLeastThreeSelectedScopes()
                    ? DropdownMenuAction.Status.Normal
                    : DropdownMenuAction.Status.Disabled);
            ...
            switch (evt.target)
            {
                ...
                {
                    evt.menu.AppendAction("Delete", delegate { DeleteSelectionCallback(AskUser.DontAskUser); },
                        (DropdownMenuAction a) => canDeleteSelection
                            ? DropdownMenuAction.Status.Normal
                            : DropdownMenuAction.Status.Hidden);
                    break;
                }
        ...
        {
            return Activator.CreateInstance(GraphProcessorEditorUtil.GetViewType(connectionVM.ModelType)) as
                BaseConnectionView;
        }
        ...
        protected virtual void UpdateInspector()
        {
            ...
            if (Selection.activeGameObject != null &&
                Selection.activeGameObject.GetComponent<IGraphAssetOwner>() != null)
                return;
        ...
        protected virtual bool IsCompatible(BasePortView fromPortView, BasePortView toPortView, NodeAdapter nodeAdapter)
        {
            if (toPortView.direction == fromPortView.direction)
                return false;
            // 类型兼容查询
            if (!toPortView.ViewModel.PortType.IsAssignableFrom(fromPortView.ViewModel.PortType) &&
                !fromPortView.ViewModel.PortType.IsAssignableFrom(toPortView.ViewModel.PortType))
                return false;
            return true;
        }
        ...
        internal void OpenNodeMenuForPort(BasePortView sourcePortView, Vector2 screenPosition)
        {
            ...
            var nodeMenu = ScriptableObject.CreateInstance<NodeMenuWindow>();
            nodeMenu.Initialize(
                $"Create & Connect ({sourcePort.Name} | {GetPortDirectionLabel(sourcePort.Direction)} | {GetPortTypeLabel(sourcePort.PortType)})",
                this);
            BuildNodeMenu(nodeMenu);
            ...
            nodeMenu.SetNodeCreatedHandler(newNode =>
            {
                ...
                if (targetPort != null)
                {
                    if (sourcePort.Direction == BasePort.Direction.Left ||
                        sourcePort.Direction == BasePort.Direction.Top)
                ...
                Context.Do(() =>
                {
                    ...
                    if (connectCommand != null && connectCommand.Connection == null)
                    {
                        ...
                        Context?.graphWindow?.ShowNotification(
                            new GUIContent("Create node succeeded, but connect failed"));
                    }
                }, () =>
                {
        ...
        private static PortProcessor FindBestCompatiblePort(BaseNodeProcessor node, PortProcessor sourcePort)
        {
            ...
            var sourceIsInput = sourcePort.Direction == BasePort.Direction.Left ||
                                sourcePort.Direction == BasePort.Direction.Top;
        ...
        public void PasteClipboard()
        {
            ...
            {
                var noteModel = (PFGraph.StickyNote)CloneModel(clip.Notes[i]);
            ...
            {
                var groupModel = (PFGraph.Group)CloneModel(clip.Groups[i]);
        ...
        private GraphClipboardData BuildClipboardFromSelection()
        {
            ...
            foreach (var item in selection)
            {
                switch (item)
                {
                    ...
                    case StickyNoteView noteView:
                        data.Notes.Add((PFGraph.StickyNote)CloneModel(noteView.ViewModel.Model));
                        break;
            ...
            {
                var group = (PFGraph.Group)CloneModel(selectedGroups[i].Model);
        ...
        {
            var target = mode == SelectionAlignMode.Right || mode == SelectionAlignMode.Bottom
                ? int.MinValue
                : int.MaxValue;
        ...
        private static void AddMoveIfChanged(Dictionary<IGraphElementProcessor_Scope, Rect> dict,
            IGraphElementProcessor_Scope scope, InternalVector2Int newPosition)
        {
