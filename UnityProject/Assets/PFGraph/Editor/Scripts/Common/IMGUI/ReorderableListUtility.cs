...
namespace PFGraph
{
    public static class ReorderableListUtility
    {
        ...
        public static ReorderableList CreateAutoLayout(SerializedProperty property, string[] headers,
            float?[] columnWidth = null, float columnSpacing = 10f)
        {
            return CreateAutoLayout(property, true, true, true, true, headers, columnWidth, columnSpacing);
        }

        public static ReorderableList CreateAutoLayout(SerializedProperty property, bool draggable, bool displayHeader,
            bool displayAddButton, bool displayRemoveButton, float columnSpacing = 10f)
        {
            return CreateAutoLayout(property, draggable, displayHeader, displayAddButton, displayRemoveButton, null,
                null, columnSpacing);
        }

        public static ReorderableList CreateAutoLayout(SerializedProperty property, bool draggable, bool displayHeader,
            bool displayAddButton, bool displayRemoveButton, string[] headers, float?[] columnWidth = null,
            float columnSpacing = 10f)
        {
            var list = new ReorderableList(property.serializedObject, property, draggable, displayHeader,
                displayAddButton, displayRemoveButton);
            var colmuns = new List<Column>();

            list.drawElementCallback =
                DrawElement(list, GetColumnsFunc(list, headers, columnWidth, colmuns), columnSpacing);
            list.drawHeaderCallback =
                DrawHeader(list, GetColumnsFunc(list, headers, columnWidth, colmuns), columnSpacing);
        ...
        public static bool DoLayoutListWithFoldout(ReorderableList list, string label = null)
        {
            ...
            property.isExpanded =
                EditorGUILayout.Foldout(property.isExpanded, label != null ? label : property.displayName, true);
        ...
        private static ReorderableList.ElementCallbackDelegate DrawElement(ReorderableList list,
            System.Func<List<Column>> getColumns, float columnSpacing)
        {
            return (rect, index, isActive, isFocused) =>
            {
                ...
                for (var ii = 0; ii < columns.Count; ii++)
                {
                    ...
                    EditorGUI.PropertyField(arect,
                        property.GetArrayElementAtIndex(index).FindPropertyRelative(c.PropertyName), GUIContent.none);
                    arect.x += arect.width + columnSpacing;
                }
            };
        }

        private static ReorderableList.HeaderCallbackDelegate DrawHeader(ReorderableList list,
            System.Func<List<Column>> getColumns, float columnSpacing)
        {
            return (rect) =>
            {
        ...
        private static System.Func<List<Column>> GetColumnsFunc(ReorderableList list, string[] headers,
            float?[] columnWidth, List<Column> output)
        {
            var property = list.serializedProperty;
            return () =>
            {
                    ...
                    if (property.isArray && property.arraySize > 0)
                    {
                        ...
                        if (it.Next(true))
                        {
                            do
                            {
                                ...
                                {
                                    var c = new Column();
                                    c.DisplayName = (headers != null && headers.Length > index)
                                        ? headers[index]
                                        : it.displayName;
                                    c.PropertyName = it.propertyPath.Substring(prefix.Length + 1);
                                    c.Width = (columnWidth != null && columnWidth.Length > index)
                                        ? columnWidth[index]
                                        : null;
