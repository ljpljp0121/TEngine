    ...
    public class ResizableArea
    {
        ...
        void Reload()
        {
            foreach (var direction in Directions)
            {
                ...
                sides[DirectionIndexMap[direction]] =
                    GetSide(direction, side, sideOffset[DirectionIndexMap[direction]]);
            }
        }
        ...
        public Rect GetSide(UIDirection direction, float width, float offset = 0)
        {
            switch (direction)
            {
                ...
                case UIDirection.Bottom:
                    return new Rect(rect.x + width / 2, rect.y + rect.height - width / 2 + offset, rect.width - width,
                        width);
                case UIDirection.Left:
                    return new Rect(rect.x - width / 2 + offset, rect.y + width / 2, width, rect.height - width);
                case UIDirection.Right:
                    return new Rect(rect.x + rect.width - width / 2 + offset, rect.y + width / 2, width,
                        rect.height - width);
                case UIDirection.TopLeft:
                    return new Rect(rect.x - width / 2 + offset, rect.y - width / 2 + offset, width, width);
                case UIDirection.TopRight:
                    return new Rect(rect.x + rect.width - width / 2 + offset, rect.y - width / 2 + offset, width,
                        width);
                case UIDirection.BottomLeft:
                    return new Rect(rect.x - width / 2 + offset, rect.y + rect.height - width / 2 + offset, width,
                        width);
                case UIDirection.BottomRight:
                    return new Rect(rect.x + rect.width - width / 2 + offset, rect.y + rect.height - width / 2 + offset,
                        width, width);
            }
            ...
            switch (evt.type)
            {
                case EventType.Repaint:
                    if (IsEnabled(UIDirection.Top))
                        EditorGUIUtility.AddCursorRect(sides[DirectionIndexMap[UIDirection.Top]],
                            MouseCursor.ResizeVertical);
                    if (IsEnabled(UIDirection.Bottom))
                        EditorGUIUtility.AddCursorRect(sides[DirectionIndexMap[UIDirection.Bottom]],
                            MouseCursor.ResizeVertical);

                    if (IsEnabled(UIDirection.Left))
                        EditorGUIUtility.AddCursorRect(sides[DirectionIndexMap[UIDirection.Left]],
                            MouseCursor.ResizeHorizontal);
                    if (IsEnabled(UIDirection.Right))
                        EditorGUIUtility.AddCursorRect(sides[DirectionIndexMap[UIDirection.Right]],
                            MouseCursor.ResizeHorizontal);

                    if (IsEnabled(UIDirection.TopLeft))
                        EditorGUIUtility.AddCursorRect(sides[DirectionIndexMap[UIDirection.TopLeft]],
                            MouseCursor.ResizeUpLeft);
                    if (IsEnabled(UIDirection.TopRight))
                        EditorGUIUtility.AddCursorRect(sides[DirectionIndexMap[UIDirection.TopRight]],
                            MouseCursor.ResizeUpRight);

                    if (IsEnabled(UIDirection.BottomLeft))
                        EditorGUIUtility.AddCursorRect(sides[DirectionIndexMap[UIDirection.BottomLeft]],
                            MouseCursor.ResizeUpRight);
                    if (IsEnabled(UIDirection.BottomRight))
                        EditorGUIUtility.AddCursorRect(sides[DirectionIndexMap[UIDirection.BottomRight]],
                            MouseCursor.ResizeUpLeft);

                    if (IsEnabled(UIDirection.MiddleCenter) && isDragging && sideDirection == UIDirection.MiddleCenter)
                        EditorGUIUtility.AddCursorRect(sides[DirectionIndexMap[UIDirection.MiddleCenter]],
