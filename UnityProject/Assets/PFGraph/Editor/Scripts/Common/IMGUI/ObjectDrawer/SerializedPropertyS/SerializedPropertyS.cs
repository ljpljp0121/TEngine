...
namespace PFGraph
{
    public class SerializedPropertyS
    {
        ...
        private SerializedPropertyS(FieldInfo fieldInfo, SerializedPropertyS parent, string name)
        {
            ...
            var att = (UnityEngine.PropertyAttribute)null;
            if (editorType == null &&
                ReflectionEx.TryGetFieldAttribute<UnityEngine.PropertyAttribute>(fieldInfo, false, out att))
        ...
        private SerializedPropertyS(FieldInfo fieldInfo, SerializedPropertyS parent) : this(fieldInfo, parent,
            string.Empty) { }
                    ...
                    if (hasChildren)
                    {
                        ...
                        foreach (var childFieldInfo in ReflectionEx.GetFields(propertyType, BindingFlags.Instance))
                        {
                            ...
                            if (!childFieldInfo.IsPublic)
                            {
                                ...
                                if (!ReflectionEx.TryGetFieldAttribute<SerializeField>(childFieldInfo, false,
                                        out var serializeField))
                                {
                                    continue;
                                }

                                // 若带有NonSerialized特性
                                if (ReflectionEx.TryGetFieldAttribute<NonSerializedAttribute>(childFieldInfo, false,
                                        out var nonSerialized))
                                {
                                    continue;
                                }
                                // 若带有HideInInspector特性
                                if (ReflectionEx.TryGetFieldAttribute<HideInInspector>(childFieldInfo, false,
                                        out var hideInInspector))
                                {
                                    continue;
                                }
                            }
                            ...
                            {
                                childrens[id] =
                                    child = new SerializedPropertyS(childFieldInfo, this, childFieldInfo.Name);
