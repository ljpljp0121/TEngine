    ...
    public static partial class EditorGUILayoutExtension
    {
        ...
        public static bool CanDraw(FieldInfo fieldInfo)
        {
            return ((!fieldInfo.IsPrivate && !fieldInfo.IsFamily) ||
                    ReflectionEx.TryGetFieldAttribute(fieldInfo, false, out SerializeField serAtt))
                   && !ReflectionEx.TryGetTypeAttribute(fieldInfo.DeclaringType, true,
                       out NonSerializedAttribute nonAtt)
        ...
        public static object DrawField(object value, GUIContent label)
        {
            ...
            if (type.IsClass || (type.IsValueType && !type.IsPrimitive))
            {
                ...
                if (DrawFoldout(hashCode, label))
                {
                    ...
                    foreach (var field in ReflectionEx.GetFields(type,
        ...
        {
            return EditorGUIExtension.DrawValue(
                EditorGUILayout.GetControlRect(true, EditorGUIExtension.GetHeight(type, label)), type, value, label);
        }
        ...
        static object DrawArrayField(Type fieldType, object value, GUIContent label)
        {
            ...
            if (DrawFoldout(value.GetHashCode(), label))
            {
                ...
                if (flag && editingArray && (GUIUtility.keyboardControl != currentKeyboardControl ||
